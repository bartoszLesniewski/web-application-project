...
<html lang="pl">
	...
	<body>
		...
		<main>
					...
					<?php foreach ($images as $image): ?>
						<div class="image">
							...
							<div class="description">
								...
								<?php endif ?>
								<label><input form="rememberForm" type="checkbox" name="remember[]" value="<?= $image['_id'] ?>" <?= $image['check'] ?>> <i>Zaznacz zdjęcie</i></label>
							</div>
						</div>
					<?php endforeach ?>
				<?php else: ?>
			...
			<?php if (count($images)): ?>
			<div class="center">
				<form id="rememberForm" method="POST">
					<input type="submit" name="rememberSubmit" value="Zapamiętaj wybrane">
				</form>
			</div>
			<?php endif ?>
